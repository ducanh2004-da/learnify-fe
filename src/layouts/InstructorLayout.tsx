...
// Responsive InstructorLayout
// - Desktop: original 3-column header
// - Mobile: hamburger -> off-canvas drawer
// - Accessibility: aria attributes, Escape to close, body scroll lock when drawer open

export default function InstructorLayout(): JSX.Element {
  ...
  const navItems = [
    { title: 'Home', path: '/instructor' },
    { title: 'Create Course', path: '/instructor/create-course' },
    { title: 'Manage Course', path: '/instructor/courses' }
  ]
        ...
        <div className="flex items-center justify-end gap-2.5 w-2/3 sm:w-1/3">
          ...
          >
            <div className="w-[22px] h-[18px] relative">
              <span className={cn('block absolute left-0 right-0 h-[2px] transition-transform duration-200 bg-black', mobileOpen ? 'top-1/2 rotate-45' : 'top-0')} />
              <span className={cn('block absolute left-0 right-0 h-[2px] transition-opacity duration-200 bg-black', mobileOpen ? 'opacity-0' : 'top-1/2')} />
              <span className={cn('block absolute left-0 right-0 h-[2px] transition-transform duration-200 bg-black', mobileOpen ? 'bottom-1/2 -rotate-45' : 'bottom-0')} />
            </div>
          </button>
        </div>

        {mobileOpen && (
          <div role="dialog" aria-modal="true" className="fixed inset-0 z-[999] flex" onClick={() => setMobileOpen(false)}>
            <div className="absolute inset-0 bg-black/40" />

            <aside className="relative z-50 w-11/12 max-w-xs bg-white h-full shadow-xl p-6 overflow-auto transform transition-transform duration-300" onClick={e => e.stopPropagation()}>
              ...
              <nav className="flex flex-col gap-4">
                {navItems.map((item, idx) => (
                  <NavLink key={idx} to={item.path} onClick={() => setMobileOpen(false)} className={({ isActive }) => cn('py-3 text-[1.15rem] rounded-md', isActive ? 'text-primary font-semibold' : 'text-black')}>
                    {item.title}
                  </NavLink>
                ))}
              </nav>
              ...
              <div className="mt-6 border-t pt-4">
                ...
                {authUser && (
                  <div className="mt-4 flex flex-col gap-2">
                    ...
                    <button onClick={() => { handleUserMenuSelect('learner'); setMobileOpen(false) }} className="text-left py-2">Back to learner page</button>
